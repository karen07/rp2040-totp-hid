...
- Красное мигание встроенного RGB LED при ошибке чтения RTC
- Желтое мигание встроенного RGB LED при установленном OSF
...
- В DS3231 установлен OSF: LED мигает желтым.
- Статус DS3231 не читается по I2C: LED мигает красным.
- При получении WebHID команды LED коротко вспыхивает синим.

После успешной установки времени через WebHID или UART прошивка очищает OSF, и желтое предупреждение прекращается.
...
- версию WebHID протокола
- major, minor и patch версии прошивки
...
панель показывает предупреждение в логе.
...
## Диагностика USB/WebHID

В этой версии используются два независимых HID-интерфейса:

- HID instance 0 / endpoint 0x81 - клавиатура
- HID instance 1 / endpoint 0x82 - vendor WebHID
  (`usagePage=0xFF00`, `usage=0x0001`)

USB PID изменён на `0x4008`, чтобы ОС и браузер не использовали закэшированный
дескриптор старой версии с объединённым HID-интерфейсом. WebGUI выбирает именно
`CAFE:4008` с vendor usage `FF00:0001`.

Индикация LED:

- красный мигает - не удаётся прочитать status-регистр DS3231;
  это ошибка RTC/I2C
- жёлтый мигает - установлен флаг OSF DS3231; RTC доступен,
  но есть предупреждение
- короткая синяя вспышка - WebHID-команда дошла до прошивки
- зелёный - нажата кнопка

Диагностический UART0 по-прежнему использует GP12 (TX) / GP13 (RX),
115200 8N1.
